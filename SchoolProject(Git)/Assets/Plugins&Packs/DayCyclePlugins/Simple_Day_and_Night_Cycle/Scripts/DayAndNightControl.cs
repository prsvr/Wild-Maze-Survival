...
public class DayAndNightControl : MonoBehaviour {
	...
	public int currentDay; //day 8287... still stuck in this grass prison... no esacape... no freedom...
    ...
    public int mytime;
    public Sprite sprite_morning, sprite_midnoon, sprite_noon,sprite_night;
    public GameObject time_icon;
    Image time_image;
	...
	void Start () {
        ...
        currentDay = GlobalManager.Instance.Day;
        time_image = time_icon.GetComponent<Image>();
        UpdateLight();
    }
    ...
    {
        currentTime += (mytime / SecondsInAFullDay) * timeMultiplier;
        if (currentTime >= 1)
        {
    ...
    void OnDisable()
    {
        GlobalManager.Instance.Day = currentDay + 1;
    }
    ...
    void TimeImage()
    {
        if (currentTime < 0.5f && currentTime > 0.2f)
        {
            time_image.sprite = sprite_morning;
        }
        if (currentTime >= 0.5f && currentTime < 0.7f)
        {
            time_image.sprite = sprite_midnoon;
        }
        if (currentTime > 0.7f && currentTime < 0.8f)
        {
            time_image.sprite = sprite_noon;
        }
        if (currentTime > 0.8f && currentTime < 1f)
        {
            time_image.sprite = sprite_night;
        }
    }
