...
fixed4 ComputeOverlayColor(float3 worldPos, float3 ray, float3 normal, float depth)
{
	...
	fixed alpha = overlayAmount * noiseMultiplier;
	...
	overlayColor.rgb = CalculateLightColorWorldSpace(p).rgb * _OverlayColor * overlayAmount;
	return fixed4(overlayColor.rgb * noiseMultiplier, alpha);
}
