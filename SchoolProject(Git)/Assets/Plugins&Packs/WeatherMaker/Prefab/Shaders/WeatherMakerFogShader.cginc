...
{	
	...
	else
	{
		...
		float3 halfDiam = 1.0 / ((_FogBoxMax - _FogBoxMin) * 0.5);
		for (int i = 0; i < samples; i++)
		{
			float xDiff = min(pos.x - _FogBoxMin.x, _FogBoxMax.x - pos.x);
			float yDiff = min(pos.y - _FogBoxMin.y, _FogBoxMax.y - pos.y);
			float zDiff = min(pos.z - _FogBoxMin.z, _FogBoxMax.z - pos.z);
			float3 diff = float3(xDiff, yDiff, zDiff);
			float heightFactor = min(1.0, (_FogBoxMax.y - pos.y) * halfDiam.y);
...
fixed4 fog_box_fragment_shader(volumetric_data i) : SV_TARGET
{
	...
	float3 startPos;
	RaycastFogBox(i.rayDir, i.normal, screenUV, depth, startPos, noise);
