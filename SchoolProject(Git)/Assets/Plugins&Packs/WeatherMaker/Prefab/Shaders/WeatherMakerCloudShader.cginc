...
fixed3 ComputeCloudColor(fixed3 cloudColor, float fbm, float3 rayDir, float3 worldPos, fixed4 sunColor, fixed alphaAccum)
{
	...
	fixed3 cloudDirLight = ComputeDirectionalLightCloud(sunColor, _WeatherMakerSunDirectionUp, rayDir, fbm, scatterMultiplier, _WeatherMakerSunLightPower.x, _WeatherMakerSunLightPower.y);
...
fixed4 ComputeCloudColor(float3 rayDir, sampler2D noiseTex, sampler2D maskTex, float2 screenUV, fixed4 sunColor, inout fixed alphaAccum)
{
	...
	// compute lighting
	fixed3 cloudColor = _CloudColor[_CloudIndex] * _WeatherMakerSkyGradientColor.rgb;
	cloudColor = ComputeCloudColor(cloudColor, fbm, rayDir, worldPos, sunColor, alphaAccum);
