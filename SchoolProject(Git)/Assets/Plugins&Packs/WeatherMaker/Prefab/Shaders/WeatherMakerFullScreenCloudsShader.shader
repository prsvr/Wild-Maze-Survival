...
Shader "WeatherMaker/WeatherMakerFullScreenCloudsShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma multi_compile __ WEATHER_MAKER_CUBEMAP
			...
			fixed4 frag (full_screen_fragment i) : SV_Target
			{
				...
				{
					_CloudIndex = 3;
					cloudColor = ComputeCloudColor(float3(cloudRay.x, cloudRay.y + _CloudRayOffset[3], cloudRay.z), _CloudNoise4, _CloudNoiseMask4, i.uv, sunLightColor, alphaAccum);
				...
				{
					_CloudIndex = 2;
					cloudColor = ComputeCloudColor(float3(cloudRay.x, cloudRay.y + _CloudRayOffset[2], cloudRay.z), _CloudNoise3, _CloudNoiseMask3, i.uv, sunLightColor, alphaAccum);
					blendClouds(cloudColor, finalColor, sunLightColor);
				}
				...
				{
					_CloudIndex = 1;
					cloudColor = ComputeCloudColor(float3(cloudRay.x, cloudRay.y + _CloudRayOffset[1], cloudRay.z), _CloudNoise2, _CloudNoiseMask2, i.uv, sunLightColor, alphaAccum);
					blendClouds(cloudColor, finalColor, sunLightColor);
				}
				...
				{
					_CloudIndex = 0;
					cloudColor = ComputeCloudColor(float3(cloudRay.x, cloudRay.y + _CloudRayOffset[0], cloudRay.z), _CloudNoise1, _CloudNoiseMask1, i.uv, sunLightColor, alphaAccum);
					blendClouds(cloudColor, finalColor, sunLightColor);
				}
