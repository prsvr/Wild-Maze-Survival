...
fixed3 ComputeWaterCaustics(float3 worldPos, float3 rayDir, float z, float2 distortOffset, fixed reducer, fixed density) // density of 0 is fully opaque
{
	...
	reducer *= CalculateDirLightShadowPower(depthPos, 0) * _WeatherMakerDirLightColor[0].a;
	reducer = pow(reducer, 4.0);

	// if rotating to light, no need to swap z and y as the rotation does it
	depthPos = RotatePointZeroOriginQuaternion(depthPos, _WeatherMakerDirLightQuaternion[0]);
	...
	return c * c * _WeatherMakerDirLightPower[0].z * _CausticsTintColor * _WeatherMakerDirLightColor[0].rgb * _WaterColor.rgb * _WaterColor.rgb;
}
...
)
{
	...
	float depth01 = WM_SAMPLE_DEPTH(refractionUV);
	...
	float aboveWater = (worldPos.y <= _WorldSpaceCameraPos.y);
	half refl2Refr = Fresnel(viewVector, worldNormal, FRESNEL_BIAS, FRESNEL_POWER) * (FRESNEL_BIAS > -100.0) * aboveWater;

#if defined(WATER_EDGEBLEND_ON)

	int tmp = (depth < refractionPos.z);
	...
	if (_CausticsScale.x > 0.0 && _CausticsScale.y > 0.0 && aboveWater)
	{
	...
	// shore foam
	if (_Foam.x > 0.0 && _Foam.y > 0.0 && _Foam.z > 0.0 && aboveWater)
	{
		// foam color
	...
	{
		// occlude base color base on amount of water
	}
...
fixed4 ApplyWaterLightForwardBaseAdd(fixed4 baseColor, fixed atten, float3 lightDir, float3 worldPos, float3 worldNormal, fixed4 specularColor, fixed specularPower)
{
	...
	p.rayDir = rayDir;
	finalColor += CalculateSpecularColor(fixed4(_LightColor0.rgb, 1.0), lightDir, atten, p);
